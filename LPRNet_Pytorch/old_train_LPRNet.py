...
from LPRNet_Pytorch.data.old_load_data import CHARS, CHARS_DICT, LPRDataLoader
...
def get_parser():
    ...
    parser.add_argument('--max_epoch', default=35, help='epoch to train the network')
    parser.add_argument('--img_size', default=[94, 24], help='the image size')
    parser.add_argument('--train_img_dirs', default="C:\\Users\\Y9000P\\Downloads\\2026ICContest\\ICcontest_project\\LPR_DATA_PERSP\\train", help='the train images path')
    parser.add_argument('--test_img_dirs', default="C:\\Users\\Y9000P\\Downloads\\2026ICContest\\ICcontest_project\\LPR_DATA_PERSP\\test", help='the test images path')
    ...
    parser.add_argument('--save_folder', default='./weights/run2/', help='Location to save checkpoint models')
    parser.add_argument('--pretrained_model', default='./weights/run1/Final_LPRNet_model.pth', help='pretrained base model')
...
def split_target_labels(labels, lengths):
    start = 0
    targets = []
...
def train():
    ...
    os.makedirs(args.save_folder, exist_ok=True)     # 保存路径目录创建
...
def Greedy_Decode_Eval(Net, datasets, args):
    ...
    for i in range(epoch_size):
        # load train data
        images, labels, lengths = next(batch_iterator)
        targets = split_target_labels(labels, lengths)
